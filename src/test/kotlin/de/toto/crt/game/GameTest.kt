...
//            (!it.nags.contains(NAG.DUBIOUS_MOVE) || it.whiteToMove) && (it.previous?.next?.indexOf(it) == 0 || !it.whiteToMove)
    ...
    @Test
    fun contains() {
        val game = fromPGN("e4 e5 Nf3 Nc6 (Nf6 Nxe5) Bb5 Nf6 *")
        assertTrue(game.contains(game.pos("Bb5")!!))
        assertTrue(game.contains(fromPGN("e4 e5 Nf3 Nc6 (Nf6 Nxe5) Bb5 Nf6 *").pos("Nf6")!!))
        assertFalse(game.contains(fromPGN("e4 e5 Nf3 Nc6 (Nf6 Nxe5) Bb5 h6 *").pos("h6")!!))
    }

    @Test
    fun mergeInVariation() {
        val game1 = fromPGN("e4 e5 Nf3 Nc6 Bb5 a6 *")
        val game2 = fromPGN("1.e4 e5 2.Nf3 Nc6 (2..Nf6 3.Nxe5) 3.Bb5 Nf6 *")
        game1.mergeIn(game2)
